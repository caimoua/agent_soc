...
- 已新增 Tool-call Detector v0.3 原型，挂在 `0x4200_3000` APB window，支持最多 8 个 16-bit token pattern 匹配、match_count 和 IRQ pending，软件镜像和 SoC directed test 已补齐，等待 VCS 确认。
...
当前支持 `smoke/core/cache/ahb/mmio/soc/isa/agent/full` 几个回归集合，日志保存在 `sim/log/regress/`。
...
Agent workload / matrix accelerator：
```bash
...
后续优先确认 Agent Matrix Accelerator v0.2a/v0.2b 和 Tool-call Detector v0.3 的 VCS 数据，然后推进 v0.4：把 `agent_matrix_irq` / `tool_call_irq` 接入 CPU trap/interrupt 路径，或先做一个小型 IRQ aggregator。同时建立 NPU/Agent Accelerator 的软件功能模型和测试向量，并继续完善 Stage A4：固定 lint warning baseline、在 Linux/CI 中接入 Verilator/Yosys，补充真实综合/时序报告。
