...
- Agent Event Counter v0.5：
  ...
  - `sim/regress/regression_list.txt` 已把该 test 接入 `agent` suite。
  - 当前 directed test 状态为 `PENDING`，等待用户在 VCS 环境确认。
...
1. 等待用户确认 `rv32i_agent_event_counter_soc_tb` 或 `agent` suite VCS PASS，再把 v0.5 verification 状态改为 PASS。
2. v0.5 PASS 后开始结构整理：把当前 demo 链路收束为 CPU subsystem、SoC fabric、Agent peripherals、software smoke、verification 五条清晰骨架。
