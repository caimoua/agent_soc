...
1. 建立 NPU / Agent Accelerator 的软件功能模型和测试向量，不直接跳到完整 32x32 NPU RTL。
2. 扩展 agent workload baseline 的结果记录，按 workload 分段统计 cycle/branch/stall。
3. 开始 v0.5：Agent event/perf counter window，用硬件记录 tool-call IRQ、matrix start/done 和 token event。
