...
- Agent matrix accelerator v0.2a：
  ...
  - 用户已通过 `agent` regression 确认 VCS PASS，日志目录：`/home2/kairos18/workspace/ai_agent_mcu_npu_soc/sim/log/regress/20260521_162353-agent`。
- Agent matrix accelerator v0.2b SRAM mode：
  - `rtl/bus/rv32i_ahb_lite_matrix_2m4s.v`
  - `rv32i_ahb_matrix_soc_top` 已从 1-master matrix 升级到 2-master front-end，CPU 为 M0，accelerator 为 M1。
  - `rv32i_agent_matrix_accel` 已新增 SRAM-mode 寄存器：`SRC_A/SRC_B/DST/STRIDE_A/STRIDE_B/STRIDE_D/FLAGS`。
  - `FLAGS.bit0=1` 时，accelerator 通过 simple-to-AHB master 读取 SRAM matrix/vector，并把 int32 result 写回 SRAM。
...
- Tool-call Detector v0.3：
  ...
  - APB window：`0x4200_3000`。
  - `rv32i_apb_periph_mux` 已新增 Tool-call Detector decode path。
  - `rv32i_ahb_matrix_apb_soc_top` 已实例化 detector 并透出 `tool_call_irq` 与 debug status/match/token outputs。
...
- Agent IRQ Aggregator v0.4：
  ...
  - 当前 CPU CSR 只实现 MTIP/MTIE，所以 v0.4 先把 `timer_irq | agent_matrix_irq | tool_call_irq` 聚合成 `cpu_timer_irq`，接入现有 machine timer interrupt 路径。
  - `rv32i_ahb_matrix_apb_soc_top` 已透出 `cpu_timer_irq` 和 `dbg_agent_irq_status`。
  ...
  - `sim/regress/regression_list.txt` 已把该 test 接入 `agent` suite。
  - 用户已通过 `agent` regression 确认 VCS PASS，日志目录：`/home2/kairos18/workspace/ai_agent_mcu_npu_soc/sim/log/regress/20260522_095831-agent`。
- Agent Event Counter v0.5：
  - `rtl/accel/rv32i_agent_event_counter.v`
  - APB window：`0x4200_4000`。
  - `rv32i_apb_periph_mux` 已新增 Agent Event Counter decode path。
  - `rv32i_tool_call_detector` 已透出 token/match/irq_clear event pulse。
  - `rv32i_agent_matrix_accel` 已透出 start/done event pulse。
  - `rv32i_ahb_matrix_apb_soc_top` 已实例化 event counter 并透出 `dbg_agent_event_*` debug counters。
...
- Agent peripheral cluster v0.6 结构整理：
  ...
  - 用户已通过 `agent` regression 确认 VCS PASS，日志目录：`/home2/kairos18/workspace/ai_agent_mcu_npu_soc/sim/log/regress/20260522_105035-agent`。
...
1. 继续结构整理：把 demo software、runtime driver、directed tests 和 Agent docs 再拆成更清楚的骨架。
