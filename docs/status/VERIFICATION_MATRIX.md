...
| Agent peripheral cluster refactor | `agent` regression suite | `bash ./regress/run_regression.sh --suite agent --keep-going` | PASS |
...
- 2026-05-22：用户确认 Agent peripheral cluster v0.6 结构整理后的 `agent` regression suite VCS PASS，日志目录为 `/home2/kairos18/workspace/ai_agent_mcu_npu_soc/sim/log/regress/20260522_105035-agent`。本轮确认 `rtl/agent/rv32i_agent_periph_cluster.v` 收束 matrix/tool/IRQ/event counter 叶子模块后，原有 agent workload、matrix、tool-call、IRQ 和 event counter 行为保持通过。
...
- 2026-05-21：新增 Tool-call Detector v0.3 SoC directed test：`rv32i_tool_call_detector_soc_tb`。该测试加载 `software/bin/tool_call_detector.memh`，配置 4-token pattern，写入 8-token stream，检查 match_count/token_count/status/IRQ clear 和无 matrix/UART 副作用；已由用户通过 `agent` regression 确认 PASS。
