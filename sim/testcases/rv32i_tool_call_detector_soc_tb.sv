...
module rv32i_tool_call_detector_soc_tb;
  ...
  initial begin
    ...
    if (!$value$plusargs("FLASH_MEMH=%s", flash_memh)) begin
      flash_memh = "../software/bin/tool_call_detector.memh";
    end
  ...
  always @(posedge clk) begin
    if (flash_hsel && flash_hwrite) begin
      $fatal(1, "unexpected flash write in tool-call detector test");
    end
    if (sram_hsel) begin
      $fatal(1, "unexpected SRAM access in tool-call detector test");
    end
    if (ahb_periph_hsel) begin
      $fatal(1, "unexpected AHB peripheral access in tool-call detector test");
    end
  end
  ...
  initial begin
    ...
    timeout = 0;
    while (!dbg_ebreak && (timeout < 2500)) begin
      @(posedge clk);
      #1ps;
      timeout = timeout + 1;
    end
    ...
    if (!dbg_ebreak) begin
      $fatal(1, "timeout waiting for tool-call detector ebreak");
    end
    if (dbg_illegal_instr || dbg_ecall ||
        dbg_cpu_bus_error || dbg_matrix_decode_error || dbg_apb_decode_error) begin
      $fatal(1, "unexpected trap or bus error in tool-call detector test");
    end

    check_reg(5'd5,  32'h0000_000b, "x5 status before irq clear");
    check_reg(5'd6,  32'd1,         "x6 match_count");
    check_reg(5'd7,  32'd8,         "x7 token_count");
    check_reg(5'd8,  32'd1,         "x8 irq_status before clear");
    check_reg(5'd9,  32'd0,         "x9 irq_status after clear");
    check_reg(5'd10, 32'h0000_0003, "x10 status after irq clear");
    check_reg(5'd29, 32'ha700_0001, "x29 tool-call signature");
    ...
    if (dbg_tool_call_status !== 32'h0000_0003 ||
      ...
      $fatal(1, "tool-call detector debug mismatch");
    end
    if (timer_irq || agent_matrix_irq || tool_call_irq) begin
      $fatal(1, "unexpected IRQ output after tool-call detector clear");
    end
    if (dbg_agent_matrix_start_count !== 32'd0) begin
      $fatal(1, "agent matrix should remain idle in tool-call detector test");
    end
    if (uart_tx_valid || (dbg_uart_tx_count !== 32'd0)) begin
      $fatal(1, "unexpected UART activity in tool-call detector test");
    end

    $display("[PASS] rv32i_tool_call_detector_soc_tb");
    ...
    $display("  match_count=%0d token_count=%0d last_token=0x%04x status=0x%08x",
             dbg_tool_call_match_count, dbg_tool_call_token_count,
             dbg_tool_call_last_token[15:0], dbg_tool_call_status);
    $finish;
  end
