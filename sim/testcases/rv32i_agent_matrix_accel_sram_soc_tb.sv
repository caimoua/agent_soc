...
module rv32i_agent_matrix_accel_sram_soc_tb;
  ...
  initial begin
    ...
    for (i = 0; i < 256; i = i + 1) begin
      sram[i] = 32'hdead_0000 + i;
    end

    if (!$value$plusargs("FLASH_MEMH=%s", flash_memh)) begin
      flash_memh = "../software/bin/agent_matrix_accel_sram.memh";
    end
  ...
  always @(posedge clk) begin
    if (flash_hsel && flash_hwrite) begin
      $fatal(1, "unexpected flash write in agent matrix accelerator SRAM test");
    end
    if (ahb_periph_hsel) begin
      $fatal(1, "unexpected AHB peripheral access in agent matrix accelerator SRAM test");
    end
    if (sram_hsel && sram_hwrite) begin
      if (sram_hsize != 3'b010) begin
        $fatal(1, "unexpected non-word SRAM write in accelerator SRAM test");
      end
  ...
  initial begin
    ...
    timeout = 0;
    while (!dbg_ebreak && (timeout < 3000)) begin
      @(posedge clk);
      #1ps;
      timeout = timeout + 1;
    end
    ...
    if (!dbg_ebreak) begin
      $fatal(1, "timeout waiting for agent matrix accelerator SRAM ebreak");
    end
    if (dbg_illegal_instr || dbg_ecall ||
        dbg_cpu_bus_error || dbg_matrix_decode_error || dbg_apb_decode_error) begin
      $fatal(1, "unexpected trap or bus error in agent matrix accelerator SRAM test");
    end
    ...
    check_reg(5'd29, 32'ha600_0003, "x29 matrix SRAM signature");
    check_reg(5'd30, 32'd0,         "x30 failure code");
    check_reg(5'd31, 32'd1,         "x31 pass marker");

    if (sram[32] !== 32'hffff_fffd ||
        sram[33] !== 32'd20 ||
        sram[34] !== 32'hffff_fffc ||
        sram[35] !== 32'd82) begin
      $fatal(1, "SRAM result window mismatch");
    end
    if (dbg_agent_matrix_result0 !== 32'hffff_fffd ||
      ...
      $fatal(1, "agent matrix debug result mismatch in SRAM mode");
    end
    ...
    if (dbg_matrix_m1_grant_count == 32'd0) begin
      $fatal(1, "accelerator AHB master did not receive any grants");
    end
    if (timer_irq || agent_matrix_irq) begin
      $fatal(1, "unexpected IRQ output in agent matrix accelerator SRAM test");
    end
    if (uart_tx_valid || (dbg_uart_tx_count !== 32'd0)) begin
      $fatal(1, "unexpected UART activity in agent matrix accelerator SRAM test");
    end

    $display("[PASS] rv32i_agent_matrix_accel_sram_soc_tb");
    ...
    $display("  ahb_grants cpu=%0d accel=%0d",
             dbg_matrix_m0_grant_count, dbg_matrix_m1_grant_count);
    $finish;
  end
